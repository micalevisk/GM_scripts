...
(function (fetch){
  ...
  function createTaskOnTodoist(content, dueDate, priority = 4) {
    ...
    }).then((resp) => { toggleFeedbackButton(); alert('tarefas registradas!'); }).catch(console.error);
  }
  ...
  function recuperarDatasDevolucao() {
    ...
    return nodes.filter((_, idx) => idx % 3 === 0) // apenas colunas de `Data de Devolução`
                .map(node => node2Date(node));
  }
